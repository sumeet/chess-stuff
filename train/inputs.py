...
OUTPUT_H5_FILENAME = "input_100k.h5"
    ...
    with gzip.open("output.chess.gz", "rb") as f:
        ...
        with h5py.File(OUTPUT_H5_FILENAME, "w") as f:
            input_board_ds = f.create_dataset(
                "input_board", (0, 12, 8, 8), maxshape=(None, 12, 8, 8), dtype="float16", compression="gzip"
            )
            input_extras_ds = f.create_dataset(
                "input_extras", (0, 7), maxshape=(None, 7), dtype="float16", compression="gzip"
            )
            output_ds = f.create_dataset(
                "output", (0, 134), maxshape=(None, 134), dtype="float16", compression="gzip"
            )
